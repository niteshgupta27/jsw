  ...
  void HomeApi() {
    ...
    productServices.gethomedata(body: requestBody).then((value) async {
      ...
      if (value.success == true) {
        // data.value=value;
        panddingList.value=value.data.where((data) =>data.fldAttendance == 1 && data.fldInvites ==1).toList();
print("panddingList${panddingList.value.length}");
        live.value = value.data.where((data) =>  data.fldAttendance != 1 || data.fldInvites !=1).toList();
  ...
  void attendeeRefesh() {
   ...
   String current= DateConverterHelper.estimatedDate(DateTime.now());
    productServices.getmisscall(cureentDate: current).then((value) async {
      isLoading.value = false;
      if (value.success == true) {
        // data.value=value;
        HomeApi();
  ...
  final result = await Get.toNamed(Routes.AddAttendee, arguments: {"data": meeting.value.toJson(),});
  if (result != null) {
    CallGetdata();
  }
//}
...
//   }}
  ...
  Future<void> editAttendee(Table_Attendee data) async {
   // if(meeting.value.fldCustomerId=="1") {
      final result = await Get.toNamed(Routes.AddAttendee, arguments: {"data": meeting.value.toJson(), "Attendees": data});
      if (result != null) {
        CallGetdata();
      }
   // }

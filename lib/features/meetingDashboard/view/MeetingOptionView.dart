    ...
    return Scaffold(
      appBar:
            AppBar(
             // backgroundColor: Colors.transparent, // Make AppBar transparent
              ...
              // ),
              title: Text(
               controller.meeting.value.fldMTypeName!,
      ...
      onPressed: () {
        Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => QrCodeScannerPage( meetingId: controller.meeting.value.fldMId!, Userid: controller.appStorage.loggedInUser.id!,),
            ));
      ...
      body: controller.isLoading.value ?  const Center(child: CircularProgressIndicator()) :Column(
        ...
        children: [
          ...
          Expanded(
            ...
            child: GridView.count(
              ...
              crossAxisSpacing: 20,
              mainAxisSpacing: 20,childAspectRatio: 1.2,
              children: [
                // _buildGridItem(Images.estimates, "Estimates", Colors.pink.shade50),
                // _buildGridItem(Images.account, "Accounts", Colors.blue.shade50),
                _buildGridItem(Images.Meeting_Start, "Meeting Start", Colors.yellow.shade50),
                if(controller.data.value.meetingInfo==1||controller.data.value.meetingPhoto==1||controller.data.value.meetingDetails==1||controller.data.value.meetingAttendees==1)
                _buildGridItem(Images.Meeting_Details, "Meeting Details", Colors.green.shade50),
                if(controller.data.value.meetingInfo==1||controller.data.value.meetingPhoto==1||controller.data.value.meetingDetails==1||controller.data.value.meetingAttendees==1)
                _buildGridItem(Images.Meeting_Photos, "Meeting Photos", Colors.purple.shade50),
                if(controller.data.value.meetingInfo==1||controller.data.value.meetingPhoto==1||controller.data.value.meetingDetails==1||controller.data.value.meetingAttendees==1)
                _buildGridItem(Images.Attendee, "Attendee List", Colors.orange.shade50),
              ],
            ),
          ),
    ...
    return InkWell(onTap: (){
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(label.tr, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        ],
      ),
    ));
  }
  ...
  Future<void> handleAction(String action) async {
    switch (action) {
      case 'meeting start':
       ...
       final result= await Get.toNamed(Routes.MeetingStart,arguments: {"data":controller.meeting.value.toJson()});
        ...
        break;
      case 'meeting photos':
        final result= await Get.toNamed(Routes.MeetingPhotos,arguments: {"data":controller.meeting.value.toJson()});
        print("Result received from AddNotes: $result");
        if(result !=null){
          controller.CallGetdata();
        }
      ...
      case 'meeting details':
        ...
        final result= await Get.toNamed(Routes.MeetingDetail,arguments: {"data":controller.meeting.value.toJson(),"meetingAttendeesCount":controller.data.value.meetingAttendeesCount,"meeting_nonparticipants_count":controller.data.value.meeting_nonparticipants_count});
        print("Result received from AddNotes: $result");
        if(result !=null){
          controller.CallGetdata();
        }
      ...
      case 'attendee list':
        print('Logging out');
        Get.toNamed(Routes.Attendee,arguments: {"data":controller.meeting.value.toJson(),"ShowAttendeesnote":controller.data.value.messageTextAttendees.isNotEmpty});
        break;
      case 'reminders':
        print('Logging out');

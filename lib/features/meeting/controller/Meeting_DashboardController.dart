...
import 'package:flutter/material.dart'; // ADDED for BuildContext, etc.
...
class MeetingDashboardController extends GetxController {
  ...
  void checkUser() async {
    ...
    productServices.CheckUser(body: requestBody).then((value) async {
      isLoading.value = false;
        ...
        if(value.data.version== appName.value){
          ...
          Get.dialog(
            AlertDialog(
              ...
              content: Text('${value.data.linkMessage}\n${value.data.appLink}'),
              actions: <Widget>[
                TextButton(
                  onPressed: () {
  ...
  static const platform = MethodChannel('com.nebula.jsw/permissions');
  String permissionStatus = "Unknown";

  Future<void> checkPermissions() async {
    final permissionService = PermissionService();
    permissionService.listenForPermissionResults(); // Ensure listener is set up first
    ...
    if (result == "Granted") {
      ...
      fetchLocation();
    } else if (result.startsWith("Denied")) { // Check for "Denied" prefix
        ...
        content: Obx(() {
          return Column(
            ...
            children: [
              ...
              SizedBox(height: 20),
              Row(mainAxisAlignment:MainAxisAlignment.spaceAround,children: [Text(sentCount.value.toString()),SizedBox(width: 5,), Expanded(
                  child: LinearProgressIndicator(valueColor:AlwaysStoppedAnimation<Color>( AppColors.primaryColor),
                    value: appStorage.CalllogArray.isEmpty
                        ? 0.0:(sentCount / appStorage.CalllogArray.length).clamp(0.0, 1.0),
                  )),SizedBox(width: 5,),Text(appStorage.CalllogArray.length.toString())],)
            ],
          );
        }),
        actions: [
  ...
  Future<void> sendBatchData() async {
    ...
    for (var record in appStorage.CalllogArray) {
      await  updatecallDetailApi(record.fldNumber,record.fldStatus,record.fldCalltime,record.fldDuration,record.fldMid);

    }


  }
  Future<void>  updatecallDetailApi(fld_number,fld_status,fld_calltime,fld_duration,fldmeeting_Id) async {
...
class PermissionService {
  static const _channel = MethodChannel("com.nebula.jsw/permissions");

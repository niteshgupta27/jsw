...
import android.os.Environment // ADD THIS LINE
import android.provider.Settings // ADD THIS LINE

class MainActivity: FlutterActivity(){
    private val CHANNEL = "com.nebula.jsw/call"
    private val CHANNEL1 = "com.nebula.jsw/permissions"
    ...
    private fun makePhoneCall(phoneNumber: String) {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.CALL_PHONE), CALL_PHONE_PERMISSION)
        } else {
...
//                                hasOutgoingCallBeenAnswered = true
    ...
    private fun sendCallStateToFlutter(state: String, phoneNumber: String?, duration: Long? = null,type:String?,date:String?) {
        val arguments = if (duration != null) {
            mapOf("state" to state, "number" to phoneNumber, "duration" to duration,"type" to type,"Date" to date)
        } else {
    ...
    private fun accessCallLogs() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            val callDuration = cursor.getString(cursor.getColumnIndex(CallLog.Calls.DURATION)).toLong()
            ...
            Log.d("CALL_LOG", "Phone Number: $phoneNumber, Duration: $callDuration seconds,$callDate")
            sendCallStateToFlutter("Calllog", phoneNumber, callDuration, TYPE, callDate )
        }
        cursor?.close()
    }

    // RENAMED and MODIFIED the original requestPermissions function
    private fun requestAllAppPermissions() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) { // Android 11 (API 30) and above
            if (!Environment.isExternalStorageManager()) {
                ...
                intent.data = uri
                startActivityForResult(intent, ALL_FILES_ACCESS_REQUEST_CODE) // Use startActivityForResult
